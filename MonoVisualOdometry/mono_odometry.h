...
public:
  ...
  int _nframes; // overall count of frames taken
  ...
  // set mask for feature detection
  void setMask(cv::Mat);
  
  // get old and new image frames 
...
//(made public) int _nframes; 	// overall count of frames taken
    ...
    bool opticalFlow;	// set(=1) for Optical Flow 
    bool _reliable;	// flag to check if VO values are reliable or not
